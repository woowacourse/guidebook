...
1. 반영 대상 섹션:
   - `content/education/insights/` — 검증된 패턴 강화 (적용 가이드·근거 로그 보강) 또는 신설
...
| 인사이트 유형 | 반영 대상 | 반영 방식 |
|--------------|-----------|-----------|
| 교육 설계 패턴 | `content/education/insights/` 해당 패턴 문서 | 패턴 강화/신설 |
...
- 교육 모델 MDX 수정은 반드시 마커 섹션 안에서만. **여는 마커와 본문 사이, 본문과 닫는 마커 사이에는 빈 줄을 둔다** — 본문이 `##`/`###` 헤딩으로 시작할 때 주석 마커에 바로 붙으면 가독성이 떨어지고 MDX 헤딩 렌더링이 불안정해진다:
  ```
