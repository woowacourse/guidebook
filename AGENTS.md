...
이 문서의 모든 콘텐츠 판단은 아래 독자 순위에서 흘러나온다. 무엇을 넣고 뺄지, 어떤 톤으로 쓸지 애매하면 1순위 독자를 기준으로 결정한다.

1. **우테코 방식을 자기 현장에 옮기려는 실천가** — 다른 교육자·코치, 그리고 스스로 우테코식으로 배우려는 자기주도 학습자. 코어 두 기둥(이해하기·적용하기)이 이들을 위한 것이다.
2. **우테코를 깊이 이해하려는 외부 독자·LLM** — `llms.txt`로 들어오는 층. 1순위를 위해 명확히 쓰면 따라오는 부산물이자, 정확한 인용으로 1순위의 롱테일을 끌어온다.
3. **지원자·예비 크루** — 서비스하되 앞세우지 않는다. 이들에겐 공식 사이트·설명회·크루 후기라는 별도 채널이 이미 있다.
4. (부차) **현재 크루·내부 운영** — `_meta.ts` 의 `display: 'hidden'` 으로 숨김 유지.

**전제:** 이 순위는 "이 문서의 성공 = 우테코 교육법의 전파·재사용"이라는 가정에 기댄다. 만약 목적이 지원자 유치·브랜딩으로 바뀌면 순위를 재설계한다(지원자를 앞으로). 이 전제가 바뀌기 전까지 위 순위를 기본값으로 둔다.
