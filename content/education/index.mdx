...
우아한테크코스 7년간의 교육 경험과, 현장에서 진행 중인 실험을 한 자리에 모은 공식문서입니다. 위에서 아래로 갈수록 추상에서 구체로 — 철학과 커리큘럼에서 출발해, 검증된 패턴을 거쳐, 오늘의 실험까지 이어집니다.
...
`⬆️` 아이콘이 붙은 항목은 승격 파이프라인의 출발점입니다. 실험 로그에서 반복 패턴이 발견되면 검증된 패턴으로, 검증된 패턴이 실전 절차로 정제되면 실험 로그 아래의 검증된 도구로 묶입니다. 패턴과 도구가 더 일반화되면 커리큘럼과 교육 철학으로 올라갑니다. 승격 과정의 세부는 [연구 사이클 워크플로우](/education/tools/research-cycle-workflow)에서 확인할 수 있습니다.
